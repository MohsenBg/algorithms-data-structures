...
private:
  static int Partition(int *array, int high, int low) {
    ...
    while (true) {
      ...
      lp++;
      while (array[lp] < array[povit])
        lp++;

      rp--;
      while (rp >= 0 && array[rp] > array[povit])
        rp--;
    ...
    swap(array[lp], array[povit]);

    return lp;
  }

  static void Marge(int *array, int *leftSide, int sizeLeft, int *rightSide,
                    int sizeRight) {
    int i = 0, j = 0, k = 0;
...
public:
  ...
  static void QuickSort(int *array, int high, int low = 0) {
    ...
    if (high > low) {
      int povit = Partition(array, high, low);
      QuickSort(array, povit - 1, low);
      QuickSort(array, high, povit + 1);
    }
  }
